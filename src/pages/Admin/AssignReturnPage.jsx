...
import { Plus, ArrowLeft, RefreshCw, Search, Package } from 'lucide-react';
import axios from 'axios';
import Header from '../../components/common/Header';
import ActionButton from '../../components/common/ActionButton';
import AgentSelector from '../../components/CreateDebrief/AgentSelector';
import ClientSelector from '../../components/AssignReturn/ClientSelector';
import ReturnOrderList from '../../components/AssignReturn/ReturnOrderList';
...
const AssignReturnPage = () => {
  ...
  const { clients, loading: loadingClients, error: clientError, refetch: refetchClients } = useClients(refreshKey);
  const { orders, loading: loadingOrders, error: orderError, refetch: refetchOrders } = useReturnOrders(refreshKey);

  // Handle return deletion signal
  useMemo(() => {
    if (location.state?.returnDeleted) {
      setRefreshKey((prev) => prev + 1);
      toast.success('Retour supprimé, commandes libérées pour réassignation');
    }
  }, [location.state]);
  ...
  const getFilteredOrders = useCallback(() => {
    if (!selectedAgent || !selectedClient) return orders;
  ...
  }, [selectedAgent, selectedClient, selectedOrders, orders]);
  ...
  const availableStats = useMemo(
    () => ({
      ...
      totalItems: filteredOrders.reduce((sum, order) => sum + (order.itemsReturned || 0), 0),
    }),
    [filteredOrders]
  );
  ...
  const selectedStats = useMemo(
    () => ({
      ...
      totalItems: selectedOrders.reduce((sum, order) => sum + (order.itemsReturned || 0), 0),
    }),
    [selectedOrders]
  );
  ...
  const handleSelectOrder = (order) => {
    ...
    toast.success(`Commande ${order.id} ajoutée au retour`);
  };
  ...
  const handleRemoveOrder = (orderId) => {
    ...
    if (removedOrder) {
      ...
      toast.success(`Commande ${orderId} retirée du retour`);
    }
  };

  const handleAssignReturn = async () => {
    if (!selectedAgent || !selectedClient || selectedOrders.length === 0) {
      toast.error('Veuillez sélectionner un client, un livreur et au moins une commande');
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        notes: briefingNote || 'Nouveau retour assigné',
      };
      const response = await axios.post(`${API_URL}/returns`, payload, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast.success(`Retour ${response.data.return.id} créé avec succès pour ${selectedClient.name}`);
      navigate('/returns');
    } catch (err) {
      toast.error(`Échec de la création du retour: ${err.response?.data?.error || err.message}`);
    } finally {
  ...
  return (
    <div className="w-full flex flex-col h-screen bg-gradient-to-br from-blue-50 via-blue-100 to-indigo-100">
      <div className="bg-white shadow-sm border-b border-gray-200 p-6">
        <Header title="Assignation de Retour" />
        {(agentError || clientError || orderError) && (
          <div className="bg-red-100 text-red-700 p-4 rounded-lg mb-4 flex items-center justify-between">
            <span>{agentError || clientError || orderError}</span>
            {retryCount < 3 && (
              <ActionButton
                onClick={handleRetry}
                icon={RefreshCw}
        ...
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          />
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Note de retour</label>
            <input
              type="text"
              value={briefingNote}
              ...
              aria-label="Saisir une note de retour"
            />
          </div>
        </div>
      </div>
              ...
              <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                ...
                {selectedClient && selectedAgent && (
                  <span className="text-sm text-gray-500">
                    pour {selectedClient.name} ({selectedAgent.name})
                  </span>
                )}
              </h2>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="space-y-1 text-sm">
                          ...
                          <div className="flex items-center gap-2">
                            <Package className="h-4 w-4 text-gray-400" />
                            <span className="font-semibold">{order.itemsReturned} articles</span>
                          </div>
                        </div>
                      </div>
                      <button
      ...
      <div className="bg-white border-t border-gray-200 p-6 flex justify-between">
        <ActionButton
          ...
          label="Retour"
          color="bg-gray-600"
          ariaLabel="Retourner aux retours"
        />
        <ActionButton
          onClick={handleAssignReturn}
          icon={Plus}
          label={loadingReturn ? 'Assignation...' : 'Assigner le Retour'}
          disabled={loadingReturn || !selectedAgent || !selectedClient || selectedOrders.length === 0}
          ariaLabel="Assigner un nouveau retour"
        />
      </div>
    </div>
  );
...
export default AssignReturnPage;