...
const EditServiceClientPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        setTimeout(() => {
          notifySuccess("L'Service Client est modifié avec success !");
        }, 500); // Délai de 500 ms pour s'assurer que le formulaire est fermé
          navigate("/users", { state: { updatedUser: response.data } });
      }
    } catch (err) {
      const errorMessage = err.response?.data?.message || "Erreur lors de la mise à jour de l'Service Clientistrateur";
  ...
  return (
    <div className="flex-1 overflow-auto relative z-10 p-6 bg-gray-100">
      <Header title="Modifier l'Service Clientistrateur" />
