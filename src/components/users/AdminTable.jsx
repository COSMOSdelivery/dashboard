...
// Fonctions de validation (same as before, included for completeness)
const validators = {
  nom: (value) => {
...
// InputField component (memoized, unchanged)
...
const AdminTable = ({
  ...
  useEffect(() => {
    const fetchAdmins = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${API_URL}/users/allAdmins`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        // Filter out undefined/null entries and ensure data consistency
  ...
  const handleBlur = (e) => {
    ...
    validateField(name, formatField(name, value));
  };
      ...
      if (field !== "role" && field !== "telephone2") {
        ...
        const error =
          field === "confirmPassword"
            ? validators[field](value, newUser.password)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        { ...newUser, role: "ADMIN" }, // Ensure role is ADMIN
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleEditUser = (user) => {
    ...
    setFormErrors({});
    setTouched({});
    onAddClick(); // Open the form in edit mode
  };
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `${API_URL}/users/updateUser/${editUser.id}`,
        { ...newUser, role: "ADMIN" },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      const updatedUser = { ...editUser, ...newUser, id: editUser.id };
      setUsers((prev) =>
      ...
      {fullWidth ? (
        ...
        >
          <motion.div className="w-full max-w-4xl p-8 relative">
            ...
            <form
              onSubmit={editUser ? handleUpdateUser : handleSubmit}
              className="space-y-4"
            >
