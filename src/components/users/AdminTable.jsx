...
const AdminTable = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Rafraîchir les admins
      fetchAdmins();

      // Afficher la notification de succès après un court délai
      setTimeout(() => {
        notifySuccess("Un nouveau admin est ajouté avec succès!");
  ...
  const handleEditInitiate = (user) => {
    navigate(`/edit-admin/${user.id}`, { state: { user } });
  };
      ...
      {fullWidth ? (
        ...
        <>
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-semibold text-black-700">Admins</h2>
